...
void loop() {
  ...
  digitalWrite(pingPin, LOW);   // beri sinyal awal dengan LOW
  ...
  duration = pulseIn(pingPin, HIGH);		  // sinyal HIGH untuk dapat durasi dalam mikro sekon
  cm = microsecondsToCentimeters(duration);   // konversi mikro sekon ke cm
  Serial.print(cm);
  Serial.print("cm");
  Serial.println();
  delay(100);
